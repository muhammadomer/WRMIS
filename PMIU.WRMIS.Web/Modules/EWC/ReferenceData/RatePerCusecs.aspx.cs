...
{
    public partial class RatePerCusecs : BasePage
    {
        //Data Members  
        List<EC_WaterCharges> lstEffWtr = new List<EC_WaterCharges>();
        ...
        protected void btnChange_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (buttonId.Equals("btnChange")) //Effluent Waters 
                {
                    ...
                    lblRateLabel.Text = "Annual Rate (Rs./Cusecs)";
                    txtRate.Text = hdfRate.Value;
                    hlAtchmntEffulent.ToolTip = hdnAttachmentEffulent.Value;
                    if (Convert.ToString(hdnAttachmentEffulent.Value) != "")
                        hlAtchmntEffulent.Visible = true;
                    else
                        hlAtchmntEffulent.Visible = false;
                    hlAtchmntEffulent.NavigateUrl = Utility.GetImageURL("EffluentWaterCharges", hdnAttachmentEffulent.Value);
                }
                else if (buttonId.Equals("btnChangeCanal")) // Canal Special Waters
                {
                    lblTitle.Text = "Canal Special Water";
                    lblRateLabel.Text = "Annual Rate (Rs./'000 Cft)";
                    txtRate.Text = hdfRateCanal.Value;
                    hlAtchmntEffulent.ToolTip = hdnAttachmentCanal.Value;
                    if (Convert.ToString(hdnAttachmentCanal.Value) != "")
                        hlAtchmntEffulent.Visible = true;
                    else
                        hlAtchmntEffulent.Visible = false;
                    hlAtchmntEffulent.NavigateUrl = Utility.GetImageURL("EffluentWaterCharges", hdnAttachmentCanal.Value + "");
                }
        ...
        protected void btnHistry_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lblTitle.Text = "Effluent Water";
                    gv.DataSource = bll_EWC.RatePerCusecs_GetList(Constants.ECWServiceType.EFFLUENT.ToString());
                }
                else if (buttonId.Equals("btnHstoryCanal")) // Canal Special Waters
                {
                    lblTitle.Text = "Canal Special Water";
                    gv.DataSource = bll_EWC.RatePerCusecs_GetList(Constants.ECWServiceType.CANAL.ToString());
                }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                EC_WaterCharges mdl = new EC_WaterCharges();

                if (lblTitle.Text.Equals("Effluent Water")) //Effluent Waters 
                {
                    //if (!string.IsNullOrEmpty(hdfRate.Value))
                    //{
                    //    if (Convert.ToDouble(hdfRate.Value) == Convert.ToDouble(txtRate.Text))
                    //        return;
                    //}
                ...
                {
                    //if (!string.IsNullOrEmpty(hdfRateCanal.Value))
                    //{
                    //    if (Convert.ToDouble(hdfRateCanal.Value) == Convert.ToDouble(txtRate.Text))
                    //        return;
                    //}
                ...
                mdl.Charges = Convert.ToDouble(txtRate.Text);
                mdl.ChargesDate = DateTime.Now;
                ...
                else if (hlAtchmntEffulent.Visible)
                {
                    mdl.Attachment = hlAtchmntEffulent.ToolTip;
                }

                //  mdl.Attachment = attachment;
                mdl.CreatedBy = (int)SessionManagerFacade.UserInformation.ID;
                mdl.CreatedDate = DateTime.Now;
                mdl.IsActive = true;

                if (bll_EWC.RatePerCusecs_AddNew(mdl))
        ...
        {
            object objDetail = bll_EWC.RatePerCusecs_GetLatestRateDetails(Constants.ECWServiceType.EFFLUENT.ToString());
            if (objDetail != null)
            {
                ...
                foreach (PropertyInfo prop in props)
                {
                    ...
                    if (prop.ToString().Contains("Rate"))
                        lblRate.Text = propValue + "";

                    if (prop.ToString().Contains("Date"))
                        lblDate.Text = propValue + "";

                    if (prop.ToString().Contains("RateNum"))
                        hdfRate.Value = propValue + "";
            ...
            objDetail = bll_EWC.RatePerCusecs_GetLatestRateDetails(Constants.ECWServiceType.CANAL.ToString());
            if (objDetail != null)
            {
                Type propertiesType = objDetail.GetType();
                IList<PropertyInfo> props = new List<PropertyInfo>(propertiesType.GetProperties());
                foreach (PropertyInfo prop in props)
                {
                    object propValue = prop.GetValue(objDetail, null);
                    if (prop.ToString().Contains("Rate"))
                        lblRateCanal.Text = propValue + "";

                    if (prop.ToString().Contains("Date"))
                        lblDateCanal.Text = propValue + "";

                    if (prop.ToString().Contains("RateNum"))
                        hdfRateCanal.Value = propValue + "";
                    if (prop.ToString().Contains("Attachment"))
