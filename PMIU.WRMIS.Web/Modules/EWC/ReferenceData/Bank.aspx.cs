...
namespace PMIU.WRMIS.Web.Modules.EWC.ReferenceData
{
    ...
    {
        //Data Members  
        List<EC_Bank> lstWorkType = new List<EC_Bank>();
        ...
        {
            lstWorkType = bll_EWC.Bank_GetList();
        ...
        protected void gv_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            try
            {
                int id = Convert.ToInt32(((Label)gv.Rows[e.RowIndex].FindControl("lblID")).Text);

                EC_Bank mdl = new EC_Bank(); mdl.ID = id;

                // check for closure work type associated with other modules 
                if ((bool)bll_EWC.Operations_Bank(Constants.CHECK_ASSOCIATION, mdl))
                {
                ...
                if ((bool)bll_EWC.Operations_Bank(Constants.CRUD_DELETE, mdl))
                {
        ...
        protected void gv_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            try
            {
                ...
                EC_Bank obj = (EC_Bank)bll_EWC.Operations_Bank(Constants.CRUD_READ, mdl);
                ...
                if (mdl.ID == 0) // Add scenario
                {
                    ...
                    status = (bool)bll_EWC.Operations_Bank(Constants.CRUD_CREATE, mdl);
                }
                else //Edit Scenario
                {
                    ...
                    status = (bool)bll_EWC.Operations_Bank(Constants.CRUD_UPDATE, mdl);
                }
