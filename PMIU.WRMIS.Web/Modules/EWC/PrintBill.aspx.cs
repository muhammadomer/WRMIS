...
{
    public partial class PrintBill : BasePage
    {
        ...
        protected void SetControlsValues()
        {
            ...
            if (currentObj != null)
            {
                ...
                if (!string.IsNullOrEmpty(ddlCircle.SelectedValue))
                    LoadDDL(DIVISION, Convert.ToInt64(currentObj.GetType().GetProperty("CircleID").GetValue(currentObj)));
        ...
        protected void LoadGrid()
        {
            try
            {
                ...
                IEnumerable<DataRow> IeBill = new Effluent_WaterChargesBLL().BillPrint(SelectedServiceType, SelectedFinancialYear, SelectedIndustryType, SelectedZoneID, SelectedCircleID, SelectedDivisionID, indName, indNo);
        ...
        protected void gv_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName.Equals("BillingDetail"))
            {
                ...
                Response.Redirect("~/Modules/EWC/BillDetail.aspx?print=true&ID=" + Convert.ToString(key["IndustryID"]) + "&BillID=" + Convert.ToString(key["BillID"]) + "&ServiceType=" + Convert.ToString(key["ServiceType"]) + "&Year=" + (ddlfinancialyear.SelectedItem == null ? "" : ddlfinancialyear.SelectedItem.Value), false);
            }
        }
