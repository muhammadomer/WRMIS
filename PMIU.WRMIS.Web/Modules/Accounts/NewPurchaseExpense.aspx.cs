...
{
    public partial class NewPurchaseExpense : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    {
                        ltlPageTitle.Text = "Edit New Purchase Expense";
                    ...
                    {
                        ltlPageTitle.Text = "Add New Purchase Expense";
        ...
        protected void txtNPPurchaseAmount_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (txtNPPurchaseAmount.Text.Trim() != string.Empty)
                {
                    double PurchaseAmount = Convert.ToDouble(txtNPPurchaseAmount.Text.Trim());

                    if (PurchaseAmount < 1 || PurchaseAmount > 999999)
                    {
                        pnlQuotation.Visible = false;
                        pnlQuotationEdit.Visible = false;
                        txtNPPurchaseAmount.Text = string.Empty;
        ...
        protected void gvQuotationEdit_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            try
            {
                ...
                if (mdlMonthlyExpenses.Attachment != null)
                {
                    ...
                    fupNPAttachBillEdit.Mode = Convert.ToInt32(Constants.ModeValue.Thumbnail);
                    fupNPAttachBillEdit.Size = 1;
                    fupNPAttachBillEdit.ViewUploadedFilesAsThumbnail(Configuration.Accounts, lstName);
                }
        ...
        private string GenerateBillNo()
        {
            ...
            BillNo = string.Format("{0}{1}-", Prefix, "NP");
        ...
        private void BindExpenseForm()
        {
            ...
            lblNPBillNo.Text = mdlMonthlyExpenses.BillNo;
            dvBillNo.Visible = true;

            txtNPBillDate.Text = Utility.GetFormattedDate(mdlMonthlyExpenses.BillDate);
            txtPurchaseItemName.Text = mdlMonthlyExpenses.PurchasedItemName;
            txtNPApprovalReference.Text = mdlMonthlyExpenses.ApprovalReference;
            txtNPPurchaseAmount.Text = Utility.GetRoundOffValueAccounts(mdlMonthlyExpenses.PurchaseAmount);

            txtNPPurchaseAmount_TextChanged(null, null);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lstNameofFiles = fupNPAttachBill.UploadNow(Configuration.Accounts, "FormCtrl", gvQuotation.Rows.Count);
                }
                else
                {
                    lstNameofFiles = fupNPAttachBill.UploadNow(Configuration.Accounts, "FormCtrl");
                }
                ...
                if (mdlMonthlyExpenses == null)
                {
                    mdlMonthlyExpenses = new AT_MonthlyExpenses()
                    {
                        ...
                        Month = Month,
                        ExpenseTypeID = (long)Constants.ExpenseType.NewPurchase,
                ...
                {
                    TotalClaim = TotalClaim - mdlMonthlyExpenses.PurchaseAmount.Value;

                    mdlMonthlyExpenses.BillDate = Utility.GetParsedDate(txtNPBillDate.Text);
                    mdlMonthlyExpenses.PurchasedItemName = txtPurchaseItemName.Text.Trim();
                    mdlMonthlyExpenses.PurchaseAmount = Convert.ToDouble(txtNPPurchaseAmount.Text);
                    mdlMonthlyExpenses.ApprovalReference = txtNPApprovalReference.Text.Trim();
