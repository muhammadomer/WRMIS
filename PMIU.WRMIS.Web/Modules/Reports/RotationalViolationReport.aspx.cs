...
{
    public partial class RotationalViolationReport : BasePage
    {
        ...
        protected void ddlSeason_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (ddlSeason.SelectedValue != "")
                {
                    ...
                    List<object> lst = new RotationalProgramBLL().GetRotationalProgramYearBySessionID(Convert.ToInt32(ddlSeason.SelectedValue));
        ...
        private ReportData GetReportData()  
        {
            ...
            string ZoneID = ddlZone.SelectedItem.Value == string.Empty ? null : ddlZone.SelectedItem.Value;
            string CircleID = ddlCircle.SelectedItem.Value == string.Empty ? null : ddlCircle.SelectedItem.Value;
            string DivisionID = ddlDivision.SelectedItem.Value == string.Empty ? null : ddlDivision.SelectedItem.Value;
            string SubDivisionID = ddlSubDivision.SelectedItem.Value == string.Empty ? null : ddlSubDivision.SelectedItem.Value;
            ...
            reportParameter = new ReportParameter("RPYear", Year);
            rptData.Parameters.Add(reportParameter);

            reportParameter = new ReportParameter("ReadingDate", Date);
            rptData.Parameters.Add(reportParameter);

            reportParameter = new ReportParameter("UserID", UserID);
            rptData.Parameters.Add(reportParameter);

            rptData.Name = ReportConstants.rptRotationalViolation;
            return rptData;
        }

        protected void btnRotationalViolation_Click(object sender, EventArgs e)
        {
            try
            {
