...
{
    public partial class ViewReceivedStockCampSite : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                try
                {
                    ...
                    //lbl_RD.Text = Session["RD"].ToString();
                    lbl_RD.Text = Calculations.GetRDText(Convert.ToInt64(Session["RD"]));

                    //Calculations.GetRDText(Convert.ToInt64(RDedit));
                    ...
                    hlBack.NavigateUrl = string.Format("~/Modules/FloodOperations/DivisionStore/SearchReceivedStock.aspx?ID={0}", 0);
        ...
        private void BindItemsGridView()
        {
            try
            {
                DataSet DS = new DivisionStoreBLL().DSViewReceivedStockCamSite(Convert.ToInt64(Session["DivisionIDD"].ToString()), Convert.ToInt64(Session["StructureTypeID"].ToString()), Convert.ToInt64(Session["StructureID"].ToString()), Convert.ToInt64(ddlItemCategory.SelectedValue), Convert.ToInt32(Session["FFPcampSiteID"].ToString()));
        ...
        protected void gvItems_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                else if (e.Row.RowType == DataControlRowType.Header)
                {
                    if (Convert.ToInt64(ddlItemCategory.SelectedItem.Value) == 4)
                    {
                        e.Row.Cells[1].Text = "Issued at Camp Site";
                        e.Row.Cells[2].Text = "Received from Camp Site";
                    }
                }
            }
            catch (Exception exp)
