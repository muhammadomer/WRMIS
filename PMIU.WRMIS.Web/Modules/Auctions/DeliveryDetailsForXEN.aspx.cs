...
{
    public partial class DeliveryDetailsForXEN : BasePage
    {
        ...
        private void BindAuctionDetailData(long _AuctionNoticeID, long _AuctionPriceID, long ApprovalAuthorityID)
        {
            try
            {
                ...
                string DeliveryStatus = new AuctionBLL().GetStatusByNoticeAndPriceID(_AuctionNoticeID, _AuctionPriceID);//Utility.GetDynamicPropertyValue(mdlAwardedAsset,"Status");
                ...
                else
                {
                    ...
                    else if (BalanceAmount == 0 && (DeliveryStatus == "Delivered"))
                    {
                        AC_AuctionStatusDetails mdlAuctionStatusDetail = new AuctionBLL().GetAuctionStatusForXEN(Convert.ToInt64(hdnAuctionNoticeID.Value), Convert.ToInt64(hdnAuctionPriceID.Value), Convert.ToInt32(SessionManagerFacade.UserInformation.DesignationID));
                        ...
                        if (mdlAuctionStatusDetail.DeliveryStatus == "Delivered")
                        {
                    ...
                    else if (SessionManagerFacade.UserInformation.DesignationID == (long)Constants.Designation.SE || SessionManagerFacade.UserInformation.DesignationID == (long)Constants.Designation.CE)
                    {
                        BindApprovalAuthorityDropDown(4);
                        AC_AuctionStatusDetails mdlAuctionStatusDetail = new AuctionBLL().GetAuctionStatusForXEN(Convert.ToInt64(hdnAuctionNoticeID.Value), Convert.ToInt64(hdnAuctionPriceID.Value), Convert.ToInt32(SessionManagerFacade.UserInformation.DesignationID));
                        ddlStatus.Enabled = false;
                        ddlStatus.Attributes.Remove("required");
                        ddlStatus.CssClass = "form-control";
                        ...
                        txtRemarks.CssClass = "form-control commentsMaxLengthRow multiline-no-resize";
                        btnSave.Enabled = false;
                        if (mdlAuctionStatusDetail != null)
                        {
                            if (BalanceAmount == 0 && (mdlAuctionStatusDetail.DeliveryStatus == "Delivered" || mdlAuctionStatusDetail.DeliveryStatus == "Pending"))
                            {
                                if (mdlAuctionStatusDetail.DeliveryStatus == "Delivered")
                                {
                                    ddlStatus.SelectedIndex = 2;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Auctions.ApprovalProcessForXEN.IsSaved = true;
                Response.Redirect("~/Modules/Auctions/ApprovalProcessForXEN.aspx?AuctionNoticeID=" + Convert.ToInt64(hdnAuctionNoticeID.Value), false);
