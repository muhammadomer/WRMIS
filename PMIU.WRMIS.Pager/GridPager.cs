...
namespace PMIU.WRMIS.Pager
{
    [ToolboxData("<{0}:GridPager runat=\"server\"></{0}:GridPager>")]
    public class GridPager : WebControl, IPostBackEventHandler, INamingContainer
    {
        ...
        private static readonly object PageCommand = new object();
        ...
        void IPostBackEventHandler.RaisePostBackEvent(string eventArgument)
        {
            ...
            string ctrlValue = Page.Request.Params["__EVENTARGUMENT"];

            if (ctrlValue.StartsWith("PG"))
            {
                int evntarg = Convert.ToInt32(eventArgument.Replace("PG", ""));
                OnPagerCommand(new CommandEventArgs(this.UniqueID, evntarg));
            }
            else
                OnCommand(new CommandEventArgs(this.UniqueID, Convert.ToInt32(eventArgument)));

            //if (ctrlName != "" && !string.IsNullOrEmpty(ctrlName))
            //{
        ...
        public event CommandEventHandler PagerCommand
        {
            add { Events.AddHandler(PageCommand, value); }
            remove { Events.RemoveHandler(PageCommand, value); }
        }

        protected virtual void OnPagerCommand(CommandEventArgs e)
        {
            CommandEventHandler clickHandler = (CommandEventHandler)Events[PageCommand];
            if (clickHandler != null) clickHandler(this, e);
        ...
        {
            string templateCell =
                "<td class=\"PagerOtherPageCells\"><a class=\"PagerHyperlinkStyle\" href=\"{0}\" title=\""
                + " " + BackToFirstClause + " " + "\"> " + FirstClause + " </a></td>";
        ...
        private string RenderLast()
        {
            string templateCell =
                "<td class=\"PagerOtherPageCells\"><a class=\"PagerHyperlinkStyle\" href=\"{0}\" title=\""
                + " " + GoToLastClause + " " + "\"> " + LastClause + " </a></td>";
            return String.Format(templateCell,
                Page.ClientScript.GetPostBackClientHyperlink(this, PageCount.ToString()));
        }
        ...
        private string RenderBack()
        {
            string templateCell =
                "<td class=\"PagerOtherPageCells\"><a class=\"PagerHyperlinkStyle\" href=\"{0}\" title=\""
                + " " + BackToPageClause + " " + (CurrentIndex - 1).ToString() + "\"> "
                + PreviousClause + " </a></td>";
            return String.Format(templateCell,
                Page.ClientScript.GetPostBackClientHyperlink(this, (CurrentIndex - 1).ToString()));
        }
        ...
        private string RenderNext()
        {
            string templateCell =
                "<td class=\"PagerOtherPageCells\"><a class=\"PagerHyperlinkStyle\" href=\"{0}\" title=\""
                + " " + NextToPageClause + " " + (CurrentIndex + 1).ToString() + "\"> " + NextClause + " </a></td>";
            return String.Format(templateCell,
                Page.ClientScript.GetPostBackClientHyperlink(this, (CurrentIndex + 1).ToString()));
        }
        ...
        {
            return "<td class=\"PagerCurrentPageCell\"><span class=\"PagerHyperlinkStyle\" "
                + GetAlternativeText(CurrentIndex) + " ><strong> "
                + CurrentIndex.ToString() + " </strong></span></td>";
        }

        private string RenderOther(int pageNumber)
        {
            string templateCell =
                "<td class=\"PagerOtherPageCells\"><a class=\"PagerHyperlinkStyle\" href=\"{0}\" "
                + GetAlternativeText(pageNumber) + " > " + pageNumber.ToString() + " </a></td>";
            return String.Format(templateCell,
                Page.ClientScript.GetPostBackClientHyperlink(this, pageNumber.ToString()));
        }

        private string RenderSSC(int pageNumber)
        {
            string templateCell =
                "<td class=\"PagerSSCCells\"><a class=\"PagerHyperlinkStyle\" href=\"{0}\" "
                + GetAlternativeText(pageNumber) + " > " + pageNumber.ToString() + " </a></td>";
            return String.Format(templateCell,
                Page.ClientScript.GetPostBackClientHyperlink(this, pageNumber.ToString()));
        }
        ...
        private string RenderGoTo()
        {
            ...
            string templateCell =
                "<td style=\"padding:1px 1px 1px 1px;\" class=\"PagerOtherPageCells\">" +
                "<table cellspacing=0 cellpadding=0 border=0><tr valign=middle>" +
                "<td>" +
                "<div class=\"GoToLabel\">&nbsp;GoTo&nbsp;</div>" +
                "</td><td>" +
                "<select class=\"GoToSelect\" width=40px name=\"ddlTes\" id=\"ddlTes\"" +
                "onchange=\"javascript:handleGoto(this);\">{0}</select>" +
                "</td><td>" +
                "<select class=\"GoToSelect\" width=40px name=\"ddlPages\" id=\"ddlPages\"" +
                "onchange=\"javascript:changeddlpages(this);\">{1}</select></td>" +
                "</tr></table>" +
                "</td>";
            ...
            {
                sb.Append(string.Format(listItemTemplate, i == CurrentIndex ? "selected=\"selected\"" +
                "class=\"GoToSelectedOption\"" : "",
                Page.ClientScript.GetPostBackClientHyperlink(this, i.ToString()), i));
            }

            //templateCell = string.Format(templateCell, sb.ToString());

            string listItemTemplateP = "<option {0} value=\"{1}\">{2}</option>";
            StringBuilder sbp = new StringBuilder();
            for (int j = 0; j < PageSizes.Length; ++j)
            {
                string jP = "PG" + PageSizes[j];
                sbp.Append(string.Format(listItemTemplateP, PageSizes[j] == PageSize ? "selected=\"selected\"" +
                "class=\"GoToSelectedOption\"" : "",
                Page.ClientScript.GetPostBackClientHyperlink(this, jP), PageSizes[j]));
            }

            return string.Format(templateCell, sb.ToString(), sbp.ToString());
        }

        //public void SetSelectedPageSizeValue(string val)
        //{
        //    val = "PG" + val;
        //    (Page.FindControl("ddlPages") as DropDownList).Items.FindByValue("val").Selected = true;
        ...
        private string RenderGoToScript()
        {
            ...
            sb.Append(@"
                                ...
                                function changeddlpages(selectObj)
                                {
                                    //javascript:__doPostBack('gvSavedSyncPager',selectObj.value);
                                    eval(selectObj.options[selectObj.selectedIndex].value);
            ...
            sb.AppendFormat("gotoImg.src = gotoElem.style.display == 'none' ? '{0}' : '{1}';",
                                    ...
                                    "Broadcom.ContactSync.Pager.Images.arr_right.gif"),
                                    Page.ClientScript.GetWebResourceUrl(this.GetType(),
                                    "Broadcom.ContactSync.Pager.Images.arr_left.gif")
                                    );
            sb.Append("}");
        ...
        private void CalculateSmartShortcutAndFillList()
        {
            ...
            {
                int calculatedValue =
                    (int)(System.Math.Round((this.PageCount * (100 / shortCutCountRounded) * i / 100) * 0.1, 0) * 10);
        ...
        private string RenderHiddenDiv()
        {
            ...
            bool hasQueryStringParam =
                !string.IsNullOrEmpty(System.Web.HttpContext.Current.Request.ServerVariables["QUERY_STRING"])
                ? true : false;
            ...
            if (hasQueryStringParam &&
                System.Web.HttpContext.Current.Request.QueryString[this.QueryStringParameterName] != null)
            {
                regEx = new Regex(this.QueryStringParameterName + @"\=\d*",
                    RegexOptions.Compiled | RegexOptions.Singleline);
                for (int i = 0; i < this.NormalModePageCount; i++)
                {
                    sb.Append(string.Format(tempHyperlink,
                                regEx.Replace(theURL.ToString(),
                                this.QueryStringParameterName + "=" + (i + this.CurrentIndex)), i + this.CurrentIndex)
                        );
                }
            }
            else
            {
                ...
                for (int i = 0; i < this.NormalModePageCount; i++)
                {
                    ...
                    sb.Append(string.Format(tempHyperlink,
                                hasQueryStringParam ? theURL.ToString() + "&" + qsParameterName : theURL.ToString()
                                + "?" + qsParameterName,
                                i + this.CurrentIndex)
                            );
                }
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            if (GenerateGoToSection)
                Page.ClientScript.RegisterClientScriptBlock(this.GetType(), this.ID, RenderGoToScript(), false);
